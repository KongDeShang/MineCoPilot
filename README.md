...
- **撤销的覆盖面有限**：
  ...
  - 撤销栈**只在内存里**（最多 20 步），**刷新即失效**。刷新后「撤销这次写入」按钮**仍然显示**（结果卡本身是从存档恢复的，你还能看到"写入过什么"），但点它会**明确拒绝**并说明原因，而不是照着一个失效的 id 去撤 —— 撤销条目 id 每次加载都从 `undo-1` 重新排，照着旧 id 撤会撤掉刷新后新写入的那一条（静默改错数据），所以这里选择"说清撤不了"。另一条更常见的路径是好的：**离开 AI 页再回来**（组件重挂载、会话未变），撤销照常可用
  - 真正的"跨刷新撤销"需要把撤销表示变成可持久化的形态（`nlActions.js` 里目前存的是闭包），涉及口述解析核心，未做
