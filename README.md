...
> 详细材料见 [`docs/参赛材料.md`](docs/参赛材料.md)，演示流程见 [`docs/演示脚本.md`](docs/演示脚本.md)，验收结论见 [`docs/测试报告.md`](docs/测试报告.md)
...
npm run self-check  # 206 条核心逻辑断言
npm run coverage    # 口述指代消解的功能覆盖矩阵
npm run e2e         # 68 条无头浏览器端到端（真实点击真实路由）
npm run e2e:nl      # 24 条口述录入端到端（含撤销、歧义、不误写）

npm run verify      # 以上全部 + 生产构建
```

当前状态：**lint 0 错 0 警 / self-check 206 通过 / e2e 68 通过 / e2e:nl 24 通过 / 运行时异常 0**。
