      ...
      <template v-else>
        ...
        <div v-if="msg.execResult" class="cmd-result" :class="{ undone: msg.execResult.undone }">
          ...
          <div class="cmd-result-actions">
            ...
            <!--
              这条撤销记录已经不在撤销栈里了（撤过了 / 被存储预算顶出去 / 导入备份或
              恢复演示数据清空过），所以这张卡上的"撤销这次写入"撤不了。
              原来按钮照常高亮，点了才弹一句"已经不能撤了" —— 诚实，但用户感知是
              bug："亮着的按钮不能用"。现在把拒绝提前到外观上：置灰 + tooltip 说明原因，
              点了没反应这件事本身就成了说明（而不是一个要等点击才知道的意外）。
            ...
            <el-tooltip
              ...
              :disabled="undoable"
              placement="top"
              content="这条写入的撤销记录不在撤销栈里了（已撤过，或被清空过）。数据不是写入前的状态，请到工单 / 维保页确认。"
            >
              <span>
                <el-button
                  type="danger"
            ...
            <span v-else class="cmd-hint">撤销记录已不在撤销栈里，不能自动撤销</span>
          </div>
        </div>
...
defineProps({
  ...
  /**
   * 这张卡上的「撤销这次写入」是否可用。
   * 由父组件判定（撤销栈在 store 里，父组件才知道这条写入的撤销记录还在不在）。
