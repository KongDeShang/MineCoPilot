...
const solutions = [
  ...
  },
  {
    ...
    // 原来这条写的是"语音说一句自动转工单，拍张照自动识别"，但本机既没接麦克风
    // 也没接 OCR（AIAssistant 里那两处只填预置样例，自己标着"演示样例"）。
    // 换成真实存在的能力：AI 助手的口述录入——一句话描述故障，
    // 五层设备指代消解认出是哪台（utils/nlCommand.js），在理解卡里确认后建单。
    // 注意不能写成"语音"：这里说的是**打字口述**，语音识别并未实现。
    // 也不能和第 1 条（Excel 多文件导入）重复，那条讲的是台账导入。
    after: '一句话描述故障，系统认出设备并建单'
  },
  {
